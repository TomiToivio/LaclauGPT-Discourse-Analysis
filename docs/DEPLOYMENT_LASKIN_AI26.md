# DEPLOYMENT_HOST AI26 realtime deployment (operational notes)

Machine: `gpu-node-01` (Linux GPU web server, 3x Tesla V100-32GB, MongoDB 8.0,
Ollama). Repo at `LACLAUGPT_REPO_ROOT` (git, tracks
...
CLI, then an ai26_runtime writer upserts into MongoDB `laclaugpt_ai26.ai26_*`
...
| `ai26-telegram.timer/.service` | 15 min | laclaugpt_ai26.events bridge → Mongo (adapter boundary to the external Telegram collector; no telethon here) |
...
Throughput on DEPLOYMENT_HOST: `OLLAMA_KEEP_ALIVE=24h` keeps all tiers resident across
...
### MongoDB
`mongod.conf`: `bindIp: 127.0.0.1,100.64.0.10` (Tailscale), `authorization:
...
`vasama_osint` (created on admin, roles PENDING — granting `read@laclaugpt_ai26`
needs `userAdmin@laclaugpt_ai26` or root credentials, Tomi-only).
...
- `ssh DEPLOYMENT_HOST` non-interactive shells can't read protected journal logs; use
...
- Mongo user creation: `userAdmin@admin` may create users with roles on
  `admin` only; `read@laclaugpt_ai26` needs `userAdmin@laclaugpt_ai26`/root.
