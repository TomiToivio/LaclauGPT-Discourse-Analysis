    ...
    def test_join_transcripts_with_legacy_identity(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _write_legacy_csv(legacy, [{
                ...
                "political_preference": "Centre right", "corrected_date": "2024-05-21",
            ...
            self.assertEqual(json.loads(row["screen_metadata_json"])["screen_metadata"]["review_status"], "proposed")
...
class RunCountryTests(unittest.TestCase):
    def test_dry_run_counts_without_llm(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            _write_legacy_csv(legacy, [{
                "new_id": "ID1", "country": "Finland", "political_preference": "Centre right",
                "corrected_date": "2024-05-21", "account_type": "Synthetic", "source_type": "Instagram",
            }])
            ...
            def fake_transcribe(manifest, videos_dir, out_path, **kw):
                ...
                out.write_text(json.dumps({"document_id": "ID1", "transcript": "fresh",
                                           "language": "fi", "screen_ocr": "@kokoomus\nKokoomus"}),
                               encoding="utf-8")
                return 1
...
class SlurmScriptTests(unittest.TestCase):
    def test_scripts_for_both_countries(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            self.assertEqual({s.name for s in scripts}, {"ep24_full_finland.sh", "ep24_full_poland.sh"})
