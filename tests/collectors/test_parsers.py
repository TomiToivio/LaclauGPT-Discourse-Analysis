...
def test_tiktok_search_accepts_list_shaped_data(tiktok_item):
    ...
    items = tiktok.capture(
        ...
        "https://www.tiktok.com/api/search/general/full/?keyword=test",
    )
    assert len(items) == 1
    assert items[0]["id"] == tiktok_item["id"]
