...
class PathsTests(unittest.TestCase):
    def test_paths_use_roots(self) -> None:
        p = legacy_mm_pipeline.paths("finland")
        self.assertEqual(p["repo_root"], Path(legacy_mm_pipeline.REPO_ROOT))
...
class RunCountryTests(unittest.TestCase):
    def test_full_chain_with_mocks(self) -> None:
        with tempfile.TemporaryDirectory() as tmp:
            ...
            fake_whisper = legacy_asr_tests_fake_model()
            ...
            with patch.object(legacy_fetch.urllib.request, "urlopen", return_value=_Resp()):
                with patch.object(legacy_mm_pipeline.legacy_asr, "transcribe_manifest", side_effect=fake_transcribe):
                    with patch.object(legacy_mm_pipeline, "ocr_video_frames", side_effect=fake_ocr):
                        with patch.object(legacy_mm_pipeline, "_load_jsonl", side_effect=[[
                            {"document_id": "ID1", "allas_url": "https://a3s.fi/x/1.mp4", "transcript": "x"}
                        ]]):
                            with patch("pipeline.run_pipeline", return_value=[]):
                                status = legacy_mm_pipeline.run_country(
...
def legacy_asr_tests_fake_model():
    class _Info:
        language = "fi"
